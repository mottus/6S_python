...
# Solar irradiance table: 1501 values from 0.25 to 4.0 µm, step 0.0025 µm
# (W m-2 µm-1); total ~1372 W m-2 across the full spectrum.
_SI = [
...
assert len(_SI) == 1501, f"Solar irradiance table has {len(_SI)} entries, expected 1501"


def solirr(wl):
    """
    Solar spectral irradiance at wavelength wl.
    ...
    Returns
    -------
    swl : float – solar irradiance (W m-2 µm-1)
    """
    pas = 0.0025
    iwl = int((wl - 0.250) / pas + 1.5)   # 1-based index → Python 0-based below
    if iwl < 1:
        print("      wavelength  less  than  0.25  micron  :\n"
              "let's take s(l)=s(0.25)", file=sys.stderr)
        return _SI[0]
    return _SI[iwl - 1]    # convert 1-based to 0-based
