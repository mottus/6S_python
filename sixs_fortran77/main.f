...
c MATTI muutujad
      ...
      real m_korgus,m_h2o,m_o,m_aot,m_dir,m_dif,m_dir2,m_dif2
      real m_seb,m_sb
      integer m_paev,m_kuu
c**********************************************************************c
c                                                                      c
c                                                                      c
c                                                                      c
...
c**********************************************************************c
 ...
 1600 read(iread,*) wlinf,wlsup
   ...
   18 goto (110,111,112,112,114,114,114,114,114,114,114,114
     ...
     s     ,131,131,131,131,131,131,131,131),iwave
...
c 219  if (iaer.lt.4) then
c        goto(230,231,232,233),iaer+1
c      else
c        if (iaer.ge.5.and.iaer.le.7) goto(234,235,236),iaer-4
c        if (iaer.eq.4)write(iwr,133)(c(i),i=1,4)
c        if (iaer.eq.8)then
c          write(iwr,134)icp
c          do i=1,icp
c            write(iwr,135)x1(i),x2(i),cij(i)
c          enddo
c        endif
c        if (iaer.eq.9)write(iwr,136)x1(1),x2(1),x3(1)
c        if (iaer.eq.10)write(iwr,137)x1(1)
c        if (iaer.eq.11)write(iwr, 132)' Sun Photometer'
c        if (iaer.eq.12)write(iwr,138)FILE2(1:i2)
c       if (iaerp.eq.1)write(iwr,139)FILE2(1:i2)
c        goto 249
c      endif
c  234 write(iwr, 132)'       Desertic'
c      goto 249
c  235 write(iwr, 132)'          Smoke'
c      goto 249
c  236 write(iwr, 132)'  Stratospheric'
c      goto 249
c  233 write(iwr, 132)'          Urban'
c      go to 249
c  232 write(iwr, 132)'       Maritime'
c      goto 249
c  231 write(iwr, 132)'    Continental'
c      goto 249
c  230 write(iwr, 1301)
c  249 continue

c --- aerosol model (concentration) ----
c      if(iaer.eq.0) write(iwr, 1401)
c      if(iaer.eq.0) goto 1112
...
c         goto(2000,2001,2002,2003,2004,2005,2006,2007,2008,2010)
c     *    ,(ibrdf+1)
c 2000    write(iwr, 190)
c         write(iwr, 187)
c     *brdfints(mu,1),robar1/xnorm1,robar2/xnorm2,albbrdf
c         goto 2009
...
c matti väljund:
      if (iwave.eq.-2) then
      write(iwr,1503) 'wl','direct','diffuse','(W/m^2/mic)'
      endif
c matti: tsykkel 91 et välja arvutada filtri
c ja spektri integraalid. tegelikult hiljem EI KASUTATA!
      m_sb=0
      m_seb=0
      do 91 l=iinf,isup
        sbor=s(l)
        ...
        m_seb=m_seb+coef
   91 continue
c matti lõpetab, edasi originaalne
...
c    output at the ground level.
        ...
        ani(2,3)=ee0
c matti väljund:
        if (iwave.eq.-2) write(iwr,1502)
     s     wl,
     s     ani(2,1)*sbor,
     s     (ani(2,2)+ani(2,3))*sbor
c oli:     
c     s     ani(2,1)*sbor*step/m_sb,
c     s     (ani(2,2)+ani(2,3))*sbor*step/m_sb
c aga step*m_sb-ga pole mõtet läbi jagada, muidu tulevad segased 
c ühikud; nüüd on energiatihedus mikromeetri kohta
c mille integreerimine annab koguenergia antud vahemikus     
        do 955 j=1,3
...
c
c      fophst=(sodray*fophsr+sodaer*fophsa)/(sodray+sodaer)
c      write(iwr, 931)'phase function     :',fophsr,fophsa,fophst
c
c      pizerr=1.
c      pizert=(pizerr*sodray+pizera*sodaer)/(sodray+sodaer)
...
c     xb=srotot/sutott/sdtott/tgasm
c     xc=sast
c         write(iwr, 940)
...
c NB! Matti privaatväljund
         write(iwr, 3535) 'mm','dd','h_s','H2O','O3','aot',
     s    'dir','dif','seb'

      if (iwave.eq.-2) then
         write(iwr, 3536)m_kuu,m_paev,m_korgus,m_h2o,m_o,m_aot,
     s    m_dir,m_dif,seb

      else
         write(iwr, 3537)m_kuu,m_paev,m_korgus,m_h2o,m_o,m_aot,
     s    m_dir,m_dif,seb
      endif
...
c pressure at ground level (174) and altitude (175)
 ...
 1502 format(F6.4,F14.7,F14.7)
 1503 format('#',A5,A14,A14,' ',A)
 3535 format('#',A3,2x,A2,2x,A5,2x,A6,2x,A6,2x,A6,2x,A8,2x,
     s  A8,2x,A7)
 3536 format('#',i3,2x,i2,2x,f5.2,2x,f6.3,2x,f6.3,2x,f6.3,2x,f8.3,2x,
     s  f8.3,2x,f7.2)
 3537 format(i4,2x,i2,2x,f5.2,2x,f6.3,2x,f6.3,2x,f6.3,2x,f8.3,2x,
     s  f8.3,2x,f7.2)
